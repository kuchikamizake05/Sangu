...
export default function LoginPage() {
  ...
  async function verifyCode() {
    ...
    try {
      ...
      const message = reason instanceof Error ? reason.message : "Kode belum dapat diverifikasi.";
      if (/name wajib|nama lengkap diperlukan/i.test(message)) { setNeedsName(true); setError("Masukkan nama lengkap untuk membuat akun."); }
  ...
  return <main className="mx-auto flex min-h-screen max-w-lg items-center px-5 py-10"><section className="w-full"><a className="text-3xl font-extrabold tracking-[-.07em] text-[#080808] no-underline" href="/login">sangu<span className="text-[#ff5113]">·</span></a><p className="mt-8 text-xs font-extrabold tracking-[.16em] text-[#9e1d0e]">AKUN PENGIRIM</p><h1 className="mt-2 text-5xl font-extrabold leading-[.92] tracking-[-.07em]">Masuk, lalu kirim pulang.</h1><p className="mt-5 max-w-sm text-[#676767]">Nomor WhatsApp kamu membantu menjaga akun dan memulihkan akses bila perangkat berganti.</p>
    <Card className="mt-8">
      ...
      {stage === "otp" && <><p className="text-lg font-extrabold">Masukkan kode SMS</p><p className="mt-1 text-sm text-[#676767]">Kode dikirim ke {phone}. Untuk mode demo, gunakan 000000.</p><div className="mt-5 grid gap-4"><Field label="Kode OTP"><TextInput aria-label="Kode OTP" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} /></Field>{needsName && <Field label="Nama lengkap" hint="Untuk membuat akun pertamamu"><TextInput aria-label="Nama lengkap" value={name} onChange={(event) => setName(event.target.value)} /></Field>}</div><Button className="mt-6" fullWidth onClick={verifyCode} disabled={busy}>{busy ? "Memverifikasi…" : "Verifikasi dan lanjutkan"}</Button><Button className="mt-2" fullWidth variant="ghost" onClick={() => setStage("phone")}>Ubah nomor</Button></>}
      {stage === "done" && <><p className="text-xs font-extrabold tracking-[.15em] text-[#278f35]">AKUN SIAP</p><h2 className="mt-2 text-3xl font-extrabold tracking-[-.05em]">Halo, {senderName}</h2><p className="mt-2 text-sm text-[#676767]">Kamu sudah masuk. Aktifkan sidik jari saat perangkat keamanan tersedia.</p><a className="mt-6 inline-flex min-h-11 w-full items-center justify-center rounded-full bg-[#ff5113] px-5 py-3 text-sm font-extrabold text-[#080808] no-underline" href="/app">Lanjut ke Beranda</a></>}
