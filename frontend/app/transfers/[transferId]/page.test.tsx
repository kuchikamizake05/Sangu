...
describe("TransferDetailPage", () => {
  ...
  it("shows the selected transfer and its backend event timeline", async () => {
    ...
    expect(screen.getByText("Uang aman di escrow")).toBeInTheDocument();
  ...
  it("shows a live SEP-24 anchor status and refreshes it while pending", async () => {
    ...
    expect(await screen.findByRole("status", { name: "Status pencairan anchor" })).toHaveTextContent("Siap membayar ke anchor");
