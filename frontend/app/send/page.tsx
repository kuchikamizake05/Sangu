...
function PreparedTransfer({ prepared, onCancel }: { prepared: PrepareSendResponse; onCancel: () => void }) {
  return <Card className="border-[#57ce43] !bg-[#eaf8e8]"><span className="inline-flex size-10 items-center justify-center rounded-full bg-white font-extrabold text-[#278f35]">✓</span><h1 className="mt-5 text-3xl font-extrabold tracking-[-.05em]">Transfer siap dikonfirmasi</h1><p className="mt-2 max-w-md text-[#356f3b]">Data transfer sudah disiapkan. Konfirmasi pengiriman dengan sidik jari dari perangkatmu.</p><div className="mt-6 rounded-2xl bg-white p-4 text-sm"><span className="block text-[#676767]">ID transfer</span><strong className="block break-all">{prepared.transferId}</strong><span className="mt-3 block text-[#676767]">Berlaku sampai</span><strong>{new Date(prepared.expiry * 1000).toLocaleString("id-ID")}</strong></div><TransactionConfirmation transferId={prepared.transferId} unsignedXDR={prepared.unsignedXDR} /><Button className="mt-3" variant="ghost" fullWidth onClick={onCancel}>Batalkan dan ubah transfer</Button></Card>;
}
